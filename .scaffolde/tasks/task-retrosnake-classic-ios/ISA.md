...
- ISC-6 partial: `SM_Phone_Body.glb` and `T_KeyLabels.webp` hashes match the website assets. WebKit at 428 CSS pixels and device ratio 3 measured a 642-pixel-wide canvas at MED and 1284 pixels at EPIC after the renderer change; both screenshots were viewed. The Simulator attract screen shows sharper phone and LCD detail. The native custom-scheme harness confirms the EPIC default and canvas ratio on Mac. The updated build has not yet been installed or assessed on a physical iPhone.
- ISC-7 open: Build 1 is installed by the principal and is Waiting for Review. Build 2 was uploaded for TestFlight processing during fidelity work, but its quality migration would override a manually selected lower setting. Signed build 3 removes that migration and was uploaded successfully at 3:58 PM; App Store Connect shows it Processing. Neither build has replaced the App Review version or reached public release.
